...
public class B2_ArrayList {
    ...
    Bài ArrayList
        ...
        lstKDK.add(3);//thêm phần tử vào danh sách
        lstKDK.add("FPL");
        lstKDK.add(true);

        //ArrayList Có Định Kiểu
        ArrayList<Integer> lstCDK = new ArrayList<>();
        lstCDK.add(8);
        lstCDK.add(5);
        lstCDK.add(7);
        lstCDK.add(2);
        //for loop
        for (int i = 0; i < lstCDK.size(); i++) {
            System.out.print(lstCDK.get(i) + " ");
        }
        System.out.println("");
        //for each
        //fore +tab
        for (Integer integer : lstCDK) {
            System.out.print(integer + " ");
        }
        System.out.println("\n Sap xep tang dan");
        Collections.sort(lstCDK);
        for (Integer integer : lstCDK) {
            System.out.print(integer + " ");
        }
        lstCDK.remove(2);
         System.out.println("\n Dao nguoc danh sach");
        Collections.reverse(lstCDK);
        for (Integer integer : lstCDK) {
            System.out.print(integer + " ");
        }
        lstCDK.clear();
